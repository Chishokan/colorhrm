...
| `shifts_matrix.php` | シフト表（admin/staff）。教室ごとに「1日〜月末 × 講師」で申請/確定シフトを一覧。確定シフトには打刻時刻・判定も表示 |
| `shifts_admin.php` | シフト申請・確定（admin/staff）。申請の個別/月間まとめて確定（確定時間・授業分を入力）・却下。講師/日付で絞り込み。staffは担当教室の講師のみ |
| `shifts_done.php` | 打刻・確定シフト（admin/staff）。確定シフトの一覧と打刻状況（出勤/退勤/判定）の確認・時間/教室/授業分の修正・削除・直接追加 |
